...
function setPause(input){
	return ({
		uri:`https://api.spotinst.io/aws/ec2/group/${input.groupId}/statefulInstance/${input.ssiID}/pause`,
		method:`PUT`,
...
module.exports.main = function main (event, context, callback) {
	...
	async.map(requestList, (singleRequest, mapCallback)=>{
		rp(singleRequest).then((res)=>{
			...
			mapCallback(null, {paused:true, id: successID})
		}).catch((err)=>{
			let failedID = err.error.request.url.split("/")[5]
			mapCallback(null, {paused:false, id: failedID})		
		})
	}, (err, asyncRes)=>{
		...
		asyncRes.forEach((singleResponse)=>{
			if(singleResponse.paused) success.push(singleResponse.id)
