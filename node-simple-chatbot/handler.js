...
const memoryJSON = require('./memory.js')
// where most of the html and all the css code is stored
const formatter = require('./formatter.js')

// Creating usable varibales from the memory and formatter files
...
Then adjust the html to refect the changes then render them through the resolve object.
...
*/
function refresh(con, html, resolve){
...
@return resolve  This object is required to be returned by the serverless framwework. Because
                 we want this to be a web app we have set the header to accept html code
*/
function getResponse(req, con, html, resolve){
	let query = ""
	let botMessage = ""
	let messageSize = ""
	//check if there is a response to the given message
	if(memory[req.query.userMessage]!=undefined){
		botMessage = memory[req.query.userMessage]
	}else{
		botMessage = "I dont have an answer for that"
	} 
	// insert the users message to the db
	query = `INSERT INTO conversation(string, player) VALUES('${req.query.userMessage}', 'user' );`
	con.query(query,(err, res)=>{
		...
		con.query(query,(err,res)=>{
			...
			con.query(query,(err, res)=>{
				...
				for(let element in res){
					if(res[element].string.length>38) messageSize = "message-large"
...
@param req       This is used to get the URL parameters in this case the user message
...
exports.main = function main (req) {
	...
	return new Promise(function(resolve, reject){
		return pool.getConnection((err,con)=>{
			if(req.query.userMessage!=undefined){
				return getResponse(req, con, html, resolve)
			}else{
				return refresh(con, html, resolve)
			}
		})
	})
};
